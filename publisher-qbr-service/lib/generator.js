...
const TABLE_KEY_MAP = {
  ...
  programactivationsnapshottable: "programActivationSnapshot",
  programactivationsnapshot: "programActivationSnapshot",
  ...
  competitoranalysistable: "competitorAnalysis",
  competitoranalysis: "competitorAnalysis",
  competitorweeklypubcommchart: "competitorWeeklyPubCommChart",
  competitorsharepubcommchart: "competitorSharePubCommChart",
  topprogramscompetitorperformancetable: "topProgramsCompetitorPerformance",
  topprogramscompetitorperformance: "topProgramsCompetitorPerformance",
  brandnewprogramstable: "brandNewPrograms",
  programconnectionstatustable: "programConnectionStatus",
  programconnectionstatus: "programConnectionStatus",
  programgapanalysistable: "programGapAnalysis",
  programgapanalysis: "programGapAnalysis",
  programgapanalysissummarytable: "programGapAnalysisSummary",
  programgapanalysissummary: "programGapAnalysisSummary",
...
const DEFAULT_UI_LABELS = {
  ...
  thankYouSubtitleTemplate: "TD Publisher Programme - {period} Performance Review",
  ...
  kpiTitleAovGrowthOffset: "Order Value Growth Partially Offsetting Volume Decline",
  kpiTitleRisingCpa: "Rising CPA",
  kpiTitleRoiTrend: "Earnings Efficiency Trend"
};
...
function buildExecutiveSummaryText(input) {
  ...
  const conv = m.conversionrate || m.convrate || {};
  const ov = m.totalordervalue || m.ordervalue || {};
  const commission = m.publishercommission || m.publcommission || {};
  const digitalWallet = m.digitalwallet || {};
  const totalEarnings = m.totalearnings || {};
  ...
  const openingLine = isMultiProgramScope
    ? `Across ${selectedProgramCount} selected programs, publisher programme performance was mixed in ${periodLabel}.`
    : `The ${programLabel} publisher programme delivered mixed results in ${periodLabel}.`;

  return cleanInlineText(
    `${openingLine} Conversions ${movementVerb(conversions)} ${conversions.variance || "N/A"} to ${conversions.current || "-"}, while conversion rate ${movementVerb(conv)} ${conv.variance || "N/A"} to ${conv.current || "-"}. Total order value ${movementVerb(ov)} ${ov.variance || "N/A"} to ${ov.current || "-"}, publisher commission ${movementVerb(commission)} ${commission.variance || "N/A"} to ${commission.current || "-"}, and total earnings ${movementVerb(totalEarnings)} ${totalEarnings.variance || "N/A"} to ${totalEarnings.current || "-"}. Digital wallet contribution ${movementVerb(digitalWallet)} ${digitalWallet.variance || "N/A"} to ${digitalWallet.current || "-"}. Full KPI breakdown follows on the next slides.`
  );
}
...
function buildProgramBreakdownTable(input) {
  const isPublisherProgram = input.analysisLevel === "publisher_program";
  const targetColumns = isPublisherProgram
    ? [
        "Program ID",
        "Market",
        "Clicks",
  ...
  if (Array.isArray(scope) && scope.length && typeof scope[0] === "object" && !Array.isArray(scope[0])) {
    const mappedRows = scope
      .map((row) => {
        ...
        const conversions = firstObjectValue(row, isPublisherProgram ? ["Conversions", "Sales", "Current Sales"] : ["Sales", "Current Sales"]);
        ...
        const publisherCommission = firstObjectValue(row, ["Publisher Commission", "Publ Commission", "Commission"]);
        const yoy = firstObjectValue(row, ["YoY Change", "OV YoY %", "Sales YoY %"]);
        return isPublisherProgram
          ? [programId, market, clicks, impressions, conversions, convRate, totalOv, publisherCommission, yoy]
          : [programId, market, clicks, impressions, conversions, convRate, firstObjectValue(row, ["AOV"]), totalOv, yoy];
      });
  ...
  if (scope && Array.isArray(scope.rows) && scope.rows.length) {
    ...
    const mappedRows = scope.rows
      .map((row) => {
        ...
        const conversions = firstRowCell(row, idx, isPublisherProgram ? ["conversions", "sales", "current sales"] : ["sales", "current sales"]);
        ...
        const publisherCommission = firstRowCell(row, idx, ["publisher commission", "publ commission", "commission"]);
        const yoy = firstRowCell(row, idx, ["yoy change", "ov yoy %", "sales yoy %"]);
        return isPublisherProgram
          ? [programId, market, clicks, impressions, conversions, convRate, totalOv, publisherCommission, yoy]
          : [programId, market, clicks, impressions, conversions, convRate, firstRowCell(row, idx, ["aov"]), totalOv, yoy];
      });
    const rows = mappedRows.filter((row) => isSelectedProgramId(row[0]));
    const fallbackRows = mappedRows.filter((row) => rowHasMetrics(row));
...
function buildKpiAnalysisBullets(input) {
  ...
  if (input.analysisLevel === "publisher_program") {
    const conversions = m.conversions || m.sales || {};
    const conversionRate = m.conversionrate || m.convrate || {};
    const orderValue = m.totalordervalue || m.ordervalue || {};
    const publisherCommission = m.publishercommission || m.publcommission || {};
    const digitalWallet = m.digitalwallet || {};
    const totalEarnings = m.totalearnings || {};
    const publisherGenerated = [
      `Conversion Momentum: Conversions ${directionWord(conversions?.varianceValue)} ${conversions?.variance || "N/A"} (${conversions?.difference || "-"}), with conversion rate ${directionWord(conversionRate?.varianceValue)} ${conversionRate?.variance || "N/A"} (${conversionRate?.difference || "-"}).`,
      `Order Value Movement: Total order value ${directionWord(orderValue?.varianceValue)} ${orderValue?.variance || "N/A"} (${orderValue?.difference || "-"}), showing the value base created by the publisher programme.`,
      `Publisher Commission Movement: Publisher commission ${directionWord(publisherCommission?.varianceValue)} ${publisherCommission?.variance || "N/A"} (${publisherCommission?.difference || "-"}), showing earning movement from active programmes.`,
      `Digital Wallet Contribution: Digital wallet contribution ${directionWord(digitalWallet?.varianceValue)} ${digitalWallet?.variance || "N/A"} (${digitalWallet?.difference || "-"}), highlighting incremental publisher earnings.`,
      `Total Earnings Movement: Total earnings ${directionWord(totalEarnings?.varianceValue)} ${totalEarnings?.variance || "N/A"} (${totalEarnings?.difference || "-"}), combining programme commission and wallet contribution.`
    ].map((line) => cleanInlineText(line)).filter(Boolean);
    const safeSupplemental = [...kpiHighlightTableBullets, ...preferredAi]
      .filter((line) => !/(^|\b)(aov|average order value|roi|return on investment|affiliate program|advertiser)(\b|$)/i.test(line))
      .filter((line) => isSafeKpiSupplement(line, m));
    const merged = [];
    [...publisherGenerated, ...safeSupplemental].forEach((line) => {
...
function canonicalPublisherSlideKey(rawKey) {
  ...
  const aliases = {
    ...
    movers_shakers_publisher_commission: "movers_commission_chart",
    movers_commission: "movers_commission_chart",
    ...
    competitor_gap: "growth_opportunity_gap",
    growth_opportunity_in_the_competitor_gap: "growth_opportunity_gap",
    top_10_competitor_funded_gaps: "top_competitor_funded_gaps",
    competitor_funded_gaps: "top_competitor_funded_gaps",
    program_connection_status_page_1: "program_connection_status_1",
    program_connection_status_1_2: "program_connection_status_1",
    program_connection_status_page_2: "program_connection_status_2",
    program_connection_status_2_2: "program_connection_status_2",
    share_within_competitor_group: "share_within_competitor_group",
    thankyou: "thank_you"
  };
...
function buildPublisherProgramDeckSpec(input, theme) {
  ...
  const metricFromMap = (...keys) => keys.map((key) => input.metricMap[key]).find(Boolean);

  const executiveCardConfig = [
    { keys: ["conversions", "sales"], label: "Conversions", iconKey: "sales", icon: "\u2630" },
    { keys: ["ordervalue", "totalordervalue"], label: "Total Order Value", iconKey: "ordervalue", icon: "\u25A4" },
    { keys: ["publcommission", "publishercommission", "commission"], label: "Publisher Commission", iconKey: "roi", icon: "\u25C6" },
    { keys: ["digitalwallet", "digitalwallets"], label: "Digital Wallet", iconKey: "ordervalue", icon: "\u25A3" },
    { keys: ["totalearnings", "earnings"], label: "Total Earnings", iconKey: "ordervalue", icon: "\u25B2" },
    { keys: ["convrate", "conversionrate"], label: "Conversion Rate", iconKey: "convrate", icon: "\u26A1" }
  ];

  const topCards = executiveCardConfig
    .map((cfg) => {
      const card = metricCard(metricFromMap(...cfg.keys));
      if (!card) return null;
  ...
  const kpiSummaryTable = tableOrPlaceholderNoRank(
    ...
    "KPI Summary Table",
    ["Period", "Conversions", "Conversion Rate", "Clicks", "Earnings per Click", "Earnings per Conversion", "Publisher Commission", "Digital Wallet", "Total Earnings"],
    8
  );
  ...
  const programBreakdownTable = boundProgramBreakdown
    ? tableOrPlaceholderNoRank(
      ...
      ["Program ID", "Program Name", "Publisher Commission", "Digital Wallet", "Total Earnings", "Conversions", "Order Value", "Publisher Commission YoY %", "Earnings YoY %", "Conversions YoY %"],
      12
    )
    : buildProgramBreakdownTable(input);

  const programActivationSnapshotTable = tableOrPlaceholderNoRank(
    boundTableForSlide(input, "program_activation_snapshot", ["programActivationSnapshot"]),
    "Program Activation Snapshot",
    ["Metric", "Total", "New", "New %"],
    8
  );
  const moversCommission = boundTableForSlide(input, "movers_commission_chart", ["moversCommissionChart", "moversCommission"]);
  ...
  const competitorAnalysisTable = tableOrPlaceholderNoRank(
    boundTableForSlide(input, "competitor_analysis", ["competitorAnalysis", "competitorSharePubCommChart"]),
    "Competitor Analysis",
    ["Publisher", "Publisher Commission PP", "Publisher Commission", "Share PP", "Share"],
    12
  );
  const topProgramsCompetitorPerformanceTable = tableOrPlaceholderNoRank(
    boundTableForSlide(input, "top_programs_competitor_performance", ["topProgramsCompetitorPerformance"]),
    "Top Programs Competitor Performance",
    ["Program ID", "Program Name", "Primary", "Comp. A", "Comp. B", "Comp. C", "Comp. D"],
    10
  );
  ...
  const brandNewProgramsTable = tableOrPlaceholderNoRank(
    ...
    "Top 10 New Programs",
    ["Program ID", "Program Name", "Conversions", "Order Value", "Publisher Commission", "Digital Wallet", "Total Earnings"],
    10
  );
  const programConnectionStatusSource = boundTableForSlide(input, "program_connection_status", ["programConnectionStatus"]);
  const programConnectionStatusBase = tableRows(programConnectionStatusSource, 80) || {
    columns: ["Program ID", "Program Name", "Connection Status"],
    rows: [["-", "-", "-"]]
  };
  const splitProgramConnectionTable = (pageIndex, pageSize = 48) => ({
    title: "Program Connection Status",
    columns: programConnectionStatusBase.columns,
    rows: programConnectionStatusBase.rows.slice(pageIndex * pageSize, (pageIndex + 1) * pageSize).length
      ? programConnectionStatusBase.rows.slice(pageIndex * pageSize, (pageIndex + 1) * pageSize)
      : [["-", "-", "-"]],
    dense: true
  });
  const programConnectionStatusPage1Table = splitProgramConnectionTable(0);
  const programConnectionStatusPage2Table = splitProgramConnectionTable(1);
  const programGapSummaryTable = tableOrPlaceholderNoRank(
    boundTableForSlide(input, "growth_opportunity_gap", ["programGapAnalysisSummary"]),
    "Competitor Gap Summary",
    ["Metric", "Value", "Detail"],
    8
  );
  const programGapAnalysisTable = tableOrPlaceholderNoRank(
    boundTableForSlide(input, "top_competitor_funded_gaps", ["programGapAnalysis"]),
    "Top 10 competitor-funded gaps",
    ["Rank", "Program / ID", "Primary status", "Competitor signal", "Pub Comm - Specified Sites Value"],
    10
  );
  const competitorShareTable = tableOrPlaceholderNoRank(
    boundTableForSlide(input, "share_within_competitor_group", ["competitorSharePubCommChart"]),
    "Share of publisher commission within competitor group",
    ["Publisher", "Publisher Commission PP", "Publisher Commission"],
    8
  );
  ...
  for (const blueprintSlide of blueprint) {
    ...
    if (key === "cover") {
      slides.push({
        ...
        summary: input.qbrFocusDetail
          ...
          : `A data-led review of ${input.client} publisher programme performance, programme activation, competitor context and next-cycle priorities.`,
    ...
    if (key === "executive_summary" || key === "kpi_tiles") {
      slides.push({
        id: "executive-summary",
        kind: "program-executive-summary",
        title: customTitle || "Executive KPI Tiles",
        headline: "",
    ...
    if (key === "kpi_summary_table") {
      slides.push({
        id: "kpi-summary",
        kind: "kpi-table",
        title: customTitle || "KPI Summary Table: Conversions & Earnings",
    ...
    if (key === "program_activation_snapshot") {
      slides.push({
        id: "program-activation-snapshot",
        kind: "publisher-table",
        title: customTitle || "Activation Status",
        subtitle: "Activation depth across joined, active and inactive programmes.",
        bullets: [],
        kpis: [],
        tables: [programActivationSnapshotTable]
      });
      continue;
    }
    ...
    if (key === "program_level_analysis") {
      slides.push({
        ...
        title: customTitle || "Program-Level Analysis: Publisher Commission",
    ...
    if (key === "kpi_highlights") {
      slides.push({
        id: "kpi-highlights",
        kind: "insights-blue",
        title: customTitle || "KPI Highlights & Business Implications",
    ...
    if (key === "movers_commission" || key === "movers_commission_chart") {
      slides.push({
        id: "movers-shakers-commission",
        kind: "publisher-table",
        title: customTitle || "Movers & Shakers: Publisher Commission",
    ...
    if (key === "competitor_analysis") {
      slides.push({
        ...
        kind: "publisher-table",
        title: customTitle || "Competitor Analysis",
        subtitle: "Primary publisher performance against anonymised comparison publishers.",
        bullets: [],
        kpis: [],
        tables: [competitorAnalysisTable]
      });
      continue;
    }

    if (key === "top_programs_competitor_performance") {
      slides.push({
        id: "top-programs-competitor-performance",
        kind: "publisher-table",
        title: customTitle || "Top Programs Competitor Performance",
        subtitle: "Primary publisher share against four comparison publishers by programme.",
        bullets: [],
        kpis: [],
        tables: [topProgramsCompetitorPerformanceTable]
      });
      continue;
    }
    ...
    if (key === "brand_new_programs") {
      slides.push({
        ...
        kind: "publisher-table",
        title: customTitle || "Top 10 New Programs",
        subtitle: "Programs joined or first active for the primary publisher in the current period.",
        bullets: [],
        kpis: [],
        tables: [brandNewProgramsTable],
        callout: "Newly activated programmes are not included in YoY comparisons until a prior-year baseline exists."
      });
      continue;
    }

    if (key === "growth_opportunity_gap") {
      slides.push({
        id: "growth-opportunity-gap",
        kind: "publisher-table",
        title: customTitle || "Growth opportunity in the competitor gap",
        subtitle: "Programs requiring activation, application or recovery.",
        bullets: [
          "Prioritise accepted and click-led programs first, then build an application pipeline for no-connection gaps."
        ],
        kpis: [],
        tables: [programGapSummaryTable]
      });
      continue;
    }

    if (key === "top_competitor_funded_gaps") {
      slides.push({
        id: "top-competitor-funded-gaps",
        kind: "publisher-table",
        title: customTitle || "Top 10 competitor-funded gaps",
        subtitle: "Programs receiving publisher commission from competitors while the primary publisher is not earning.",
        bullets: [],
        kpis: [],
        tables: [programGapAnalysisTable]
      });
      continue;
    }

    if (key === "program_connection_status" || key === "program_connection_status_1") {
      slides.push({
        id: "program-connection-status-1",
        kind: "publisher-table",
        title: customTitle || "Program Connection Status (1/2)",
        subtitle: "Programme connection status for the current activation scope.",
        bullets: [],
        kpis: [],
        tables: [programConnectionStatusPage1Table]
      });
      continue;
    }

    if (key === "program_connection_status_2") {
      slides.push({
        id: "program-connection-status-2",
        kind: "publisher-table",
        title: customTitle || "Program Connection Status (2/2)",
        subtitle: "Programme connection status for the current activation scope.",
        bullets: [],
        kpis: [],
        tables: [programConnectionStatusPage2Table]
      });
      continue;
    }

    if (key === "share_within_competitor_group") {
      slides.push({
        id: "share-within-competitor-group",
        kind: "publisher-table",
        title: customTitle || "Share Within Competitor Group",
        subtitle: "Publisher commission share for the primary publisher and four comparison publishers.",
        bullets: [],
        kpis: [],
        tables: [competitorShareTable]
      });
      continue;
    }
    ...
    if (key === "sales_growth_risk_dependencies" || key === "risks_dependencies") {
      slides.push({
        ...
        title: customTitle || "Risks & Dependencies",
        subtitle: "Key risks and dependencies for the next review cycle.",
        bullets: [],
        kpis: [],
