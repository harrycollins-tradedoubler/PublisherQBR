...
function payloadValues(payload, keys) {
  const nestedPayload = payload && typeof payload.payload === "object" && payload.payload
    ? payload.payload
    : {};
  return keys.flatMap((key) => [payload?.[key], nestedPayload[key]])
    .map((value) => String(value || "").trim())
    .filter(Boolean);
}

function validatePublisherTemplateBoundary(payload) {
  const templateValues = payloadValues(payload, [
    "presentonTemplateId",
    "presenton_template_id",
    "templateId",
    "template_id",
    "gammaThemeId",
    "gamma_theme_id",
  ...
  if (templateValues.some((value) => /advertiser/i.test(value))) {
    return "Publisher QBR service rejected an advertiser template field.";
  }

  const serviceUrls = payloadValues(payload, [
    "publisherPptxApiUrl",
    "publisher_pptx_api_url",
    "publisherPresentonApiUrl",
    "publisher_presenton_api_url",
    "presentonApiUrl",
    "presenton_api_url",
    "gammaApiUrl",
    "gamma_api_url",
    "advertiserPptxApiUrl",
    "advertiser_pptx_api_url"
  ]);
  if (serviceUrls.some((value) => /:3011(?:\D|$)/.test(value))) {
    return "Publisher QBR service rejected a PPTX service URL on port 3011. Use the publisher service on port 3010.";
  }
  ...
  return http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && pathname === "/generate") {
      ...
      try {
        ...
        const boundaryError = validatePublisherTemplateBoundary(payload);
        if (boundaryError) {
          json(res, 400, {
            success: false,
